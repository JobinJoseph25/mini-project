...
<div class="body-content">
  <div class="module">
    ...
    <form class="form" action="addtravel.php"  method="POST" enctype="multipart/form-data" autocomplete="off">
  ...
  <?php
  include("connect.php");
      $sql = "SELECT * From tbladdvehicle";
      $result=mysqli_query($con,$sql);
 
      while($row=mysqli_fetch_array($result))
       
           echo "<option value='" . $row['model_name'] . "'>" . $row['vid'] . $row['model_name'] ."</option>";
   ?>
</select>
<input type="text" placeholder="Amount" name="amount" required />
<input type="text" placeholder="No_of_Seats" name="noofseats" required />



      <input type="submit" value="ADD Vehicle" name="Click05" class="btn btn-block btn-primary" />
    </form>
  </div>
</div>
...
include("connect.php");
  ...
  if(isset($_POST["Click05"]))
  {
    ...
    $time=$_POST["time"];
     $amount=$_POST["amount"];
      $No_of_seat=$_POST["noofseats"];

    $v="SELECT vid FROM tbladdvehicle WHERE usrid=$userid";
    $res=mysqli_query($con,$v);
    while ($row=mysqli_fetch_array($res)) {
      $vvid=$row['vid'];
    }

       $carname=$_POST['vehicletype'];

$sql="INSERT INTO tbltraveldetails(uid, vid, from1, To1, Date1, Time1, Carname, Amount, No_of_seats) 
      VALUES ('$userid','$vvid','$From1','$To1','$Date','$time','$carname','$amount','$No_of_seat')";
